...
class Filter extends Component {
  ...
  get eventType() {
    ...
    // const { list = [] } = assetsIp.listAssetGroup
  ...
  renderSimpleForm() {
    ...
    let ASSETS = [];
    const data = this.props.assetsIp.listAsset.map(v=>{ASSETS.push({ 'value': (v.id).toString(), 'label': (v.asset_group_name).toString() })});

    return (
      <Row gutter={24}>
        <Col {...ColProps}>
          <FilterItem label={i18n.t`主机名称`}>
            {getFieldDecorator('title', { initialValue: filter.host })(
