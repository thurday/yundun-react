...
class Index extends PureComponent {
  ...
  get scoreProps() {
    const { dispatch, situation} = this.props
    let score = situation.getScore.list.score
    return {
      score
    }
  }
  ...
  get completedProps() {
    const { dispatch, situation } = this.props
    const { chartType } = situation
    // const { list } = situation.typeEvent
    const traffic_trend = situation.trafficTrend.list
    return {
      // data: completed,
      traffic_trend,
  ...
  get lineChartProps() {
    ...
    const { chartSafeType } = situation
    const { list } = situation.typeEvent
    const threat_type = list.map((v) => ({ value: v.num, name: v.eventtype }))
    
    const safety_Trend = situation.safetyTrend.list
    return {
      // data: completed,
      threat_type, // 威胁类型
      safety_Trend,//安全趋势
  ...
  trafficTrendFn = () => {
    ...
    dispatch({ type:'situation/trafficTrend' })
  }
  ...
  componentDidMount= e =>{
    ...
    this.scrollDown();
    scrollInterval=setInterval(this.scrollDown, 2000);
    this.trafficInterval = setInterval(this.trafficTrendFn,3600000);
  }
  ...
  get numberCardsProps() {
    ...
    var numbers = [...list].map((v) => ({ ...numberCardsMap[v.level], number: v.num }));
    return {
      numbers,
    }
  }
  ...
  render() {
    ...
    return (
      <Page inner pageStyle={pageStyle}>
        <Header {...this.headerProps} style={{height:'50px'}} />
        <Row>
          <Col xl={6} lg={6}>
            <Row>
              <Col xl={24}><GaugeChart {...this.scoreProps} /></Col>
